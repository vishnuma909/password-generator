...
function passwordgenerator() {
    ...
    _r = caps.filter((e, i, arr)=> {
        return (i === x || i === x+(Math.round(Math.PI)) || i === x-(Math.round(Math.E)));
    });
    if(x>nums.length) {
        x = x-nums.length;
    }
    _n = nums.filter((e, i, arr)=> {
        return (i === x || i === x+(Math.round(Math.SQRT1_2)) || i === x-(Math.round(Math.SQRT2)));
    });
    _d = small.filter((e, i, arr)=> {
        return (i === y || i === y+(Math.round(Math.PI)) || i === y-(Math.round(Math.E)));
    });
    _m = (([..._r,..._d,..._n,..._s]).toString()).replace(/,/g,'');
    (()=> {
        let arr = _m.split('').sort();
        let ci = arr.length, temp, ri;
        while (0 !== ci) {
